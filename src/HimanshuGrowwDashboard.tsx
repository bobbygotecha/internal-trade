...
const himanshuApi = { baseUrl: API_CONFIG.HIMANSHU_BASE_URL };
const himanshuUserId = 5;

const himanshuTheme = createTheme({
  palette: {
    mode: 'light',
...
export default function HimanshuGrowwDashboard() {
  ...
  const fetchPositions = async () => {
    ...
    setLoading(true);
    try {
      const rows = await stockService.getFuturesTransactions(himanshuUserId, 200, himanshuApi);
  ...
  const handleClosePosition = async (id: string) => {
    ...
    try {
      await stockService.closeFuturesOrder(id, himanshuApi);
  ...
  const handleWebhookSubmit = async () => {
    ...
    try {
      await stockService.sendFuturesWebhook(webhookForm, himanshuApi);
  ...
  return (
    <ThemeProvider theme={himanshuTheme}>
      <CssBaseline />
          ...
          {page === 'strategy' && (
            <Card sx={{ p: 3 }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                ...
                <Box component="span" sx={{ fontFamily: 'monospace', fontSize: '0.8rem' }}>himanshu.gotecha.shop</Box>.
              </Typography>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 2, mb: 2 }}>
                ...
                <FormControl fullWidth>
                  ...
                  >
                    <MenuItem value="TEST_HIMANSHU">TEST_HIMANSHU</MenuItem>
